...
export default function PermanentDrawerLeft({ create }) {
    ...
    async function fetchData() {
      try {
        ...
        if (!namedConnectionsResponse.ok || !nodeResponse.ok) {
          throw new Error(
            `HTTP error! status: ${namedConnectionsResponse.status} or ${nodeResponse.status}`
          );
        }
  ...
  function handleNodeSend(event) {
    fetch(`http://localhost:3000/api/node/${event}`)
      .then((response) => {
        if (!response.ok) {
              ...
              <AccordionDetails>
                <form id="form" action={handleCreate}>
                  <div className="flex flex-col mx-8">
                    <input
                      className="border rounded text-black my-1"
                      id="name"
                      name="name"
                    ...
                    <input
                      ...
                      placeholder="State"
                    ></input>
                    <Button color="secondary" className="p-1 mt-3" type="submit">
                      Add Node
                    </Button>
                  </div>
                </form>
          ...
          {state.person && (
            <div className="flex flex-col items-center py-5">
              <h1 className="text-2xl">
                {nodeList.singleNode.firstName} {nodeList.singleNode.lastName}
              </h1>
              <p>Age: {nodeList.singleNode.age}</p>
              <p>
                Location: {nodeList.singleNode.city} {nodeList.singleNode.state}
              </p>
              <div>
                <Button className="py-1" color="secondary">
                  Edit
                </Button>
      ...
      <Box component="main" sx={{ marginTop: `64px`, ml: `${drawerWidth}px` }}>
        <div className="flex justify-around columns-2">
          <div>
            ...
            {state.chart && !state.profile && <MyGraph sendNode={handleNodeSend}></MyGraph>}
          </div>
          <div style={{ height: "600px", width: "200px" }}>
                  ...
                  <Table>
                    {nodeList.nodeListArray.map((item) => (
                      <TableRow className="flex flex-row items-center " key={item.id}>
                        ...
                        <TableCell className="text-1xl">{item.firstName}</TableCell>
                      </TableRow>
                    ))}
                  </Table>
                </div>
            ...
            {state.connectButton && !state.node && (
              <div className="h-full overflow-y-auto">
                <h1 className="text-2xl font-bold sticky top-0 bg-stone-950 z-10 p-4">
                  Connections
                </h1>
                <Divider />
                <Table>
